...
// Custom comparator for double values, using an epsilon to account for floating-point precision.
...
// A Pattern is stored as an int where each letter's feedback is encoded in 2 bits.
...
// The pattern uses 2 bits per letter, so position 'index' uses bits [2*index, 2*index+1].
...
static bool extractWords(const string &fileName, unordered_set<string> &wordDict) {
...
// Wordle::LoadDictionary: Loads words from the WordNet files (using 'extractWords')
// and stores only words of the correct length (wordSize) into the class dictionary.
...
// Wordle::GetNextBestGuess: Calculates the entropy for each valid word and returns the word
// with the highest entropy (i.e., the one expected to yield the most information).
...
auto Wordle::PlayRandomGame() -> optional<string> {
    ...
    for (int guesses = 0; guesses < wordSize + 1 && guess != wordToGuess; ++guesses) {
...
// Wordle::PlayOffLineGame: Interactive offline mode where the user can enter commands
// to play the game. The user can provide a word, request guesses, remove words from the
// dictionary, and input feedback patterns.
void Wordle::PlayOffLineGame() {
    ...
    do {
        ...
        // If a valid command was entered (other than "remove" or "quit") and lastGuess is set:
        if (!command.empty() && !lastGuess.empty() && command != "remove" && command != "quit") {
            vector<string> patterns = promptPatterns();
            if (patterns.empty()) continue;
            Pattern p = buildPattern(patterns);
            if (p != allGreen) {
                validWords = FilterAllWordsBestOnPattern(p, lastGuess, validWords, wordSize);
                lastGuess = GetNextBestGuess(validWords);
...
namespace Testing {
    // TestGeneratePattern: Verifies that GeneratePattern returns the expected pattern
    // for given guess and actual word combinations.
    void TestGeneratePattern() {
        ...
        vector<TestCase> testCases = {
            ...
            {"CRANE", "BLAME", 0b0000100010},  // Expected pattern: Only 'E' is Green (adjust accordingly)
            ...
            {"APPLE", "PLATE", 0b0101000110},  // Expected pattern: P & L Yellow, E Green
        ...
        for (const auto &test : testCases) {
            ...
            assert(result == test.expected && "Test case failed in TestGeneratePattern!");
        }
        cout << "✅ All GeneratePattern tests passed!" << endl;
    }

    // TestCalculateEntropy: Checks that CalculateEntropy returns correct entropy values
    // for known distributions of patterns.
    void TestCalculateEntropy() {
        ...
        unordered_map<Pattern, int> case2 = {
            {242, 50},
            {0, 50}
        };
        ...
        unordered_map<Pattern, int> case3 = {
            {242, 70},
            {0, 20},
            {27, 10}
        };
        ...
        unordered_map<Pattern, int> case4 = {
            {242, 25},
            {0, 25},
            {27, 25},
            {81, 25}
    ...
    // TestPlayRandomWordleGame: Simulates 100 random games and prints the success and failure counts.
